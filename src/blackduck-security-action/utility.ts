...
export function getDefaultSarifReportPath(sarifReportDirectory: string, appendFilePath: boolean): string {
  ...
  return !appendFilePath ? path.join(pwd, constants.INTEGRATION_LOCAL_DIRECTORY, sarifReportDirectory) : path.join(pwd, constants.INTEGRATION_LOCAL_DIRECTORY, sarifReportDirectory, constants.SARIF_DEFAULT_FILE_NAME)
}
