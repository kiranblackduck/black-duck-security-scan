  ...
  getFormattedCommandForPolaris(githubRepoName: string): string {
    ...
    if (!isPrEvent) {
      if (parseToBoolean(inputs.POLARIS_REPORTS_SARIF_CREATE)) {
        ...
        // }
        isNullOrEmptyValue(inputs.POLARIS_REPORTS_SARIF_FILE_PATH) ? constants.INTEGRATION_POLARIS_DEFAULT_SARIF_FILE_PATH.trim() : inputs.POLARIS_REPORTS_SARIF_FILE_PATH.trim()
    ...
    const stateFilePath = path.join('/Users/renukac/Documents/input_json/', BridgeToolsParameter.POLARIS_STATE_FILE_NAME)
